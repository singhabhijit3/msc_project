...
if run_opt == 1:
    lstm_model.fit_generator(train_data_generator.generate(), len(train_data)//(batch_size*num_steps), num_epochs,
                        ...
                        validation_steps=len(valid_data)//(batch_size*num_steps), callbacks=[earlystopping, reduce_lr],max_queue_size=500, workers=2, use_multiprocessing=True)#, callbacks=[checkpointer])
